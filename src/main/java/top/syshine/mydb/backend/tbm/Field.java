...
/**
 * @Author: 石烨
 * @Date: 2023/02/18/16:11
 * @Description:
 */

import com.google.common.primitives.Bytes;
import top.syshine.mydb.backend.im.BPlusTree;
import top.syshine.mydb.backend.parser.statement.SingleExpression;
...
public class Field {
    ...
    public static Field loadField(Table tb, long uid) {
        byte[] raw = null;
        try {
            raw = ((TableManagerImpl)tb.tbm).vm.read(TransactionMangerImpl.SUPER_XID, uid);
        } catch (Exception e) {
            Panic.panic(e);
        }
        assert raw != null;
